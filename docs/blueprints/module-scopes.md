...
- Security posture: every external provider key (Stripe, Predis, ElevenLabs, Groq, Apify) comes from env vars, server-side only. The leaked Apify keys were rotated 2026-08-25; no key of any provider is ever written into the repo. No `.env.example`, no hardcoded secrets.
