...
-- ============================================================
--
-- El DROP SCHEMA borró la fila de profiles, pero el usuario sigue vivo en
-- el esquema auth. Esto la vuelve a crear buscando por correo, para no
-- copiar el UUID a mano.

INSERT INTO public.profiles (id, full_name, phone, role, points)
SELECT
    u.id,
    COALESCE(u.raw_user_meta_data->>'full_name', 'Eduardo'),
    COALESCE(NULLIF(u.raw_user_meta_data->>'phone', ''), '4442715470'),
    'admin',
    0
FROM auth.users u
...
BEGIN
    ...
    IF v_admin = 0 THEN
        RAISE EXCEPTION 'No quedo ningun admin. Revisa que el correo coincida con tu cuenta en auth.users';
    END IF;

    RAISE NOTICE 'Listo: % productos, % categorias, % admin',
